...
import { Card, Divider, Drawer, Grid, Link, Stack, Typography, useTheme } from '@mui/material';
import Norecordfoundcomponents from '../_Essentials/NoRecordFound';
import CloseIcon from '@mui/icons-material/Close';

const InrDeposit_table_Mbl = ({ historyData }) => {
    ...
    <>
      {historyData ? (
        historyData?.length === 0 ? (
          ...
          historyData?.filter(row => row.transType === "Deposit")?.map((row, index) => {
            return (
              <>
                <Card
                  key={index}
...
export default InrDeposit_table_Mbl
