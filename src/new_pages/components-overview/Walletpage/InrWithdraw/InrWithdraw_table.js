...
function InrWithdrawTable({ historyData }) {
  const theme = useTheme();
          ...
          {historyData ? (
            historyData.length === 0 ? (
              ...
              historyData?.filter(row => row.transType === "Withdraw")?.map((row, index) => {
                ...
                const { transAmount, planType, txid, userId, crypto, transType, transDesc, highlight, amount, charges, address, txId, href, status, date, timeStamp } = row;
...
export default InrWithdrawTable;